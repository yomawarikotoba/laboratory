...
# 5.5 Regularrization in Neural Networks  
## Introduction  
ニューラルネットワークにおける入出力数は、一般的にはデータ集合の次元によって決まるが、隠れユニットの数Mは予測性能を最適化するために調整できる自由パラメータである。Mはネットワーク内のパラメータ数（重みやバイアス）を制御しており最大尤度法を使った設定では、ある最適なMの値が存在すると予想される。これは、汎化性能が最も良くなる、つまり過学習と過小学習のバランスが取れた値。  
一般化誤差は、Mの関数として単純ではない。これは図5.10に示されるように誤差関数に局所的な最小値が存在するためである。ここでは、様々な値のMに対して複数のランダムな初期値を用いて重みベクトルを初期化することで学習を複数回行った結果が示されている。最良の検証セット性能は\\(M=8\\)の特定の解で得られたとある。実際にはMを選択する一つのアプローチは図5.10のようなグラフをプロットして、最小の検証誤差を持つモデルを選ぶことである。  
（結局この図が言いたいことは  
1.重みの初期値によって性能が大きくばらつく  
隠れユニット数（x軸）に対して、30回のランダム初期化を行い、それぞれのテスト誤差をy軸にプロット、  
結果同じネットワーク構造（例えば隠れユニットが8）でも初期値によって最終的な誤差が大きく変動していることがわかる  
これは誤差関数に局所最小が多く存在することを意味し、最適な重みが得られない可能性を示唆している  
2.隠れユニット数が増えても誤差が小さくなるとは限らない  
隠れユニット数が増えても誤差が小さくならないケースが多く見られる  
特にM=8あたりでは誤差の分布が広くなっており、過学習や複雑性による最適化の難化が起きている可能性がある  
） 
しかし、過学習を避けるためにモデルのっ複雑さを制御するほかの方法もある。第一章の多項式カーブフィッティングの議論からわかるように別のアプローチとしては比較的大きなMを選び、その後誤差関数に正則化項を追加することでモデルの複雑さを制御するという方法がある。（一生に関してはあまり詳しくは見ることができていないですすみません、、、）  
最も単純な正則化項は２乗であり、正則化された誤差関数は次のようになる。  
\\(\tilde(E(w))=E(w)+\frac{\lambda}{2}w^{T}w\\)  
この正則化は**重み減衰**としても知られており、第３章で詳しく説明されているらしい。（簡潔に言うとモデルの過学習を防ぐための手法の一つで重みの値が大きくなりすぎるのを防ぐために重みの大きさに比例したペナルティ\\(\lambda\\)を損失関数に与える。）モデルの実際の複雑さは正則化係数\\(\lambda\\)によって決定される。先に述べたようにこの正則化項は重みベクトルwに対するゼロ平均のガウス事前分布の負の対数とみなすことができる。  
## 5.5.1 Consistent Gaussian priors(一貫したガウス事前分布)  
単純な重み減衰の限界の一つは、便利であるが、入力データのスケールを変える（例えば値を１０倍にするなど）とネットワークの動きとこのルールの相性が悪くなる。これを説明するために２層の隠れユニットと線形出力ユニットを持つ多層パーセプトロンネットワークを考える。このネットワークは入力変数\\(x_{i}\\)の集合から出力変数\\(y_{k}\\)への写像を行う。  
第１隠れ層のユニットの活性化は以下の形を取る。  
\\(z_{j}=h(\sum_{i}w_{ji}x_{i}+w_{j0})\\)  
出力ユニットの活性化は次のように表される。  
\\(y_{k}=\sum_{j}w_{kj}z_{j}+w_{k0}\\)  
ここで入力データに対して次のような線形変換を施すと仮定する。  
\\(x_{i} \rightarrow \tilde(x_{i})=ax_{i}+b\\)  
それではネットワークによって実行される写像を変更せずに保つために、入力から隠れ層のユニットへの重みとバイアスの線形変換を次のように行うことができる。  
\\(w_{ji} \rightarrow \tilde(w_{ji})=\frac{1}{a}w_{ji}\\)  
\\(w_{j0} \rightarrow \tilde(w_{j0})=\frac{-b}{a}\sum_{i}w_{ji}+w_{j0}\\)  
同様にネットワークの出力変数に対する線形変換を  
\\(y_{k} \rightarrow \tilde(y_{k})=cy_{k}+d\\)  
とすると、第二層（出力層）の重みとバイアスを次のように変換することができる  
\\(w_{kj} \rightarrow \tilde(w_{kj})=cw_{kj}\\)  
\\(w_{k0} \rightarrow \tilde(w_{k0})=cw_{k0}+d\\)  
もし１つのネットワークを元のデータで訓練し、もう一つのネットワークを上記のように入力または出力のスケーリングを変えたデータで訓練した場合、変換された重み以外は同等の動作をするネットワークになるべきである。  
つまり一貫性があるということは、こうした線形変換に対してもネットワークの意味や性能が変化しないことを指す。  
どんな正則化手法も、この性質と一致しているべきである。そうでないとある会を他の等価な解よりも恣意的に優先することになるからである。  
実際式5.112の単純な重み減衰（すべての重みとバイアスに同じようにペナルティーを課す方法）はこのスケーリングに対する一貫性の性質を満たしていない。  
ここまでの話を要約すると  
- 入出力のスケールを変えたときに、同じようなネットワークにたどり着けるのが理想  
- しかし単純な重み減衰はそういった一貫性を持っていない  
- よってどんなスケーリングにも対応できる正則化手法が望ましい  
したがって、式2.116, 117, 119, 120における線形変換に対して不変な生息校を探すことにする。  
これらの変換が意味するのは、重みのスケーリングやバイアスのシフトに対して、生息加工が変化しないようにする必要があるということ。このような条件を満たす正則化項は以下の式で与えられる。  
\\(\frac{\lambda_{1}}{2}\sum_{w \in W_{1}}w^{2}+\frac{_lambda_{2}}{2}\sum_{w \in W_{2}}w^{2}\\)  
ここで\\(W_{1}\\)は第一層の重みの集合、\\(W_{2}\\)は第２層の重みの集合をそれぞれ表しており、バイアスは和には含まれていない。  
この正則化項は重みの変換を行っても以下のように正則化パラメータを再スケーリングすることで不変性を保つことができる。  
\\(\lambda_{1} \rightarrow \sqrt{a}*\lambda_{1}\\)  
\\(\lambda_{2} \rightarrow a*\lambda_{2}/{\sqrt{c}}\\)  
正則化項(5.121)は  
