...
\\(\frac{W(W+1)}{2}\\) 個の独立な要素を持っています（\\( \mathbf{H} \\) は対称行列であるためです）。 ここで \\( W \\) は重みベクトル \\( \mathbf{w} \\) の次元、つまりネットワーク中の全ての学習可能なパラメータ数です。  
...
\\(w^{(\tau+1)}=a^{(\tau)}-\eta\nablaE_n(w^{(\tau)})\\)  
この更新はデータを順番にあるいは置き換えありでランダムに選んで巡回することで繰り返される。なおデータ店のバッチに基づいて更新を行う中間的な手法も存在する。  
